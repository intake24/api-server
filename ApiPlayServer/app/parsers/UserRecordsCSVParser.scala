...
object UserRecordsCSVParser {
  ...
  private def parseRow(rowIndex: Int, headerFormat: HeaderFormat, row: Array[String]): Either[String, NewRespondent] =
    if (row.length < 2)
      ...
      if (containsWhitespace(userName) || containsWhitespace(password))
        ...
        Right(NewRespondent(userName, password, name, email, phone, customFields))
    }

  private def parseRows(headerFormat: HeaderFormat, rows: Seq[Array[String]]): Either[String, Seq[NewRespondent]] = {
    val z: Either[String, Seq[NewRespondent]] = Right(Seq())
  ...
  private def validateUniqueness(userRecords: Seq[NewRespondent]): Either[String, Unit] = {
  ...
  def parseFile(file: File): Either[String, Seq[NewRespondent]] = {
