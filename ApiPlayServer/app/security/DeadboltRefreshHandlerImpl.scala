...
class DeadboltRefreshHandlerImpl(val env: Environment[Intake24ApiEnv]) extends DeadboltHandler with JWTHandlerUtil {
    ...
    getSubject(request).map {
      subj =>
        if (subj.isDefined)
          ...
          Results.Unauthorized.withHeaders(("WWW-Authenticate", "X-Auth-Token"))
    }
}
