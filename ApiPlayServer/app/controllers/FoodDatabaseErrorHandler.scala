...
trait FoodDatabaseErrorHandler extends Results {
  ...
  def translateDatabaseError(error: AnyError): Result = error match {
    ...
    case RecordNotFound(_) => NotFound(write(ErrorDescription("RecordNotFound", "Object does not exist"))).as(ContentTypes.JSON)
    ...
    case ParentRecordNotFound(_) => BadRequest(write(ErrorDescription("ParentRecordNotFound", "An object referenced by this object does not exist, check referenced object codes"))).as(ContentTypes.JSON)
