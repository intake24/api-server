...
public class StateManagerUtil {
	...
	public static Option<Survey> getLatestState(String userName, String scheme_id, String version_id, PortionSizeScriptManager scriptManager,
	...
	public static void saveSameAsBefore(String userName, Meal meal, EncodedFood mainFood, String scheme_id, String version_id) {
		...
		localStorage.setItem(key, sameAsBeforeCodec.encode(new SerialisableSameAsBefore(mainFood, linkedFoods, scheme_id, version_id)).toString());
	}

	public static Option<SameAsBefore> getSameAsBefore(String userName, String foodCode, String scheme_id, String version_id,
			...
			try {
				...
				if (decoded.scheme_id != scheme_id || decoded.version_id != version_id) {
					log.warning("Version mismatch for same as before (" + foodCode + "): stored version is (" + decoded.scheme_id + ", "
							+ decoded.version_id + "), runtime version is (" + scheme_id + ", " + version_id + "). Ignoring record.");
					return Option.none();
				} else
	...
	public static void saveState(Storage storage, String key, Survey survey, String scheme_id, String version_id) {
		...
		String serialised = surveyCodec.encode(new SerialisableSurvey(survey, scheme_id, version_id)).toString();
