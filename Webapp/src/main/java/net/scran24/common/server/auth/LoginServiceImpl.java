...
public class LoginServiceImpl extends RemoteServiceServlet implements LoginService {
	...
	private UserInfo mkUserInfo(Subject subject) throws DataStoreException {
		...
		if (userId.survey.equals("admin"))
			// Not too elegant, but there is no reasonable survey state that can be substituted 
			return new UserInfo(userId.username, userId.survey, null, dataStore.getUserData(userId.survey, userId.username));
		else
			return new UserInfo(userId.username, userId.survey, dataStore.getSurveyParameters(userId.survey), dataStore.getUserData(userId.survey, userId.username));
	}
	...
	private UserInfo loginWithOneOf(List<ScranAuthToken> tokens) throws LoginServiceException {		
		...
		log.info("User " + user_id + " log in attempt failed due to invalid credentials (supplied password: " + hidePassword(tokens.get(0).password) + ")") ;
