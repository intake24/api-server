...
public class GenUserServiceImpl extends RemoteServiceServlet implements GenUserService {
	...
	public UserRecord autoCreateUser(final String survey_name) {
		if (survey_name.equals("admin"))
			throw new RuntimeException("This feature is not applicable to admin users");
		...
		try {
			...
			while (retries > 0) {
				...
				try {
					dataStore.addUser(survey_name, new SecureUserRecord(username, passwordHashBase64, passwordSaltBase64, roles, permissions,
			...
			return new UserRecord(username, password, new HashMap<String, String>());
