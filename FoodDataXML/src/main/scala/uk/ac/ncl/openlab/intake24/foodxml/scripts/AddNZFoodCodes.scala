...
object AddNZFoodCodes extends App {
  ...
  val updated = foods.map {
    food =>
      if (map.contains(food.code)) {
        ...
        food.copy(nutrientTableCodes = food.nutrientTableCodes + ("NZ" -> map(food.code)))
      } else
        food
  }
  ...
  updated.foreach {
    food =>
      if (food.nutrientTableCodes.isEmpty)
        println("No nutrient table code: " + food.code)
      if (food.nutrientTableCodes.get("NZ").nonEmpty && food.nutrientTableCodes.get("NDNS").nonEmpty)
