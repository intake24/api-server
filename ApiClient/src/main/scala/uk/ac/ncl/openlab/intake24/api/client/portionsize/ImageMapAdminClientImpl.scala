...
class ImageMapAdminClientImpl(apiBaseUrl: String, authCache: AuthCache) extends ImageMapAdminClient with ApiResponseParser with HttpRequestUtil with JsonParser {
  ...
  def createImageMap(baseImage: Path, svgImage: Path, sourceKeywords: Seq[String], params: NewImageMapWithObjectsRequest): Either[ApiError, Unit] = authCache.withAccessToken {
    accessToken =>
