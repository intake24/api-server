...
public interface DataStore {
	...
	/**
	 * Applies a side-effectful function to each of the surveys in the given range.
	 * The callback is to avoid keeping all surveys in memory at once (can be a large data set). 
	 * @throws IOException
	 */
	public void processSurveys (String survey_id, long timeFrom, long timeTo, Callback1<NutritionMappedSurveyRecordWithId> processSurvey) throws DataStoreException;
	...
	/**
	 * For each of the given food codes, the popularity counter must be returned from the backing data store.
	 * For food codes that do not yet have a record created via <i>incrementPopularityCount</i> zero must be returned.
	 * 
	 * <b>The resulting map MUST have entries for each of the supplied food codes.</b>
	 */
	public Map<String, Integer> getPopularityCount (Set<String> foodCodes) throws DataStoreException;
	
	/**
	 * For each of the given food codes, a "popularity" counter (the number of times that food was chosen
	 * by users) must be incremented by 1 (note that same food codes may appear more than once). If there 
	 * is no record yet for a given code, a new record must be created with an initial value of 0 and immediately
	 * incremented by 1. 
	 */
