...
public class UserRecordCSV {
    ...
    public static List<UserRecord> fromCSV(InputStream input) throws IOException {
        ...
        for (int i = 0; i < header.length; i++) {
            if (header[i].toLowerCase().equals(NAME_HEADER)) nameIndex = i;
            if (header[i].toLowerCase().equals(EMAIL_HEADER)) emailIndex = i;
            if (header[i].toLowerCase().equals(PHONE_HEADER)) phoneIndex = i;
        }
