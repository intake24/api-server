...
case class V18_RemappedGuideImage(id: String, url: String, sourcePath: String, mainImagePath: String, overlayPaths: Seq[(Long, String)])

object FoodV18_Guide_Prepare extends App with WarningMessage with DatabaseConnection {
  ...
  if (version != versionFrom) {
    ...
    val remapped = rows.map {
      row =>
        V18_RemappedGuideImage(row.id, row.base_image_url,
