...
SELECT
  v.food_code, v.locale_id, psm.id, psm.method, psm.description, COALESCE(pi.path, psm.image_url) AS image_url, psm.use_for_recipes,
       ...
       FROM v LEFT JOIN foods_portion_size_methods AS psm ON psm.food_code=v.food_code AND psm.locale_id=v.locale_id
              ...
              LEFT JOIN drinkware_sets AS dw ON (psm.method='drink-scale' AND par.name='drinkware-id' AND par.value=dw.id)
              LEFT JOIN image_maps AS dwim ON dwim.id = dw.guide_image_id
              LEFT JOIN processed_images AS pi ON ass.selection_image_id = pi.id OR gi.selection_image_id = pi.id OR dwim.base_image_id = pi.id
ORDER BY param_id
