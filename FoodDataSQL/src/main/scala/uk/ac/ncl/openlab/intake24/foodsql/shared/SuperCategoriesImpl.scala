...
trait SuperCategoriesImpl extends FirstRowValidation with HeaderRows {
  protected def foodAllCategoriesImpl(code: String)(implicit conn: java.sql.Connection): Either[LookupError, Set[String]] = {
    val query =
  ...
  def categoryAllCategoriesImpl(code: String)(implicit conn: java.sql.Connection): Either[LookupError, Set[String]] = {
    val query =
      """"|WITH RECURSIVE t(code, level) AS (
          ...
          |  (SELECT category_code as code, level + 1 as level FROM t JOIN categories_categories ON subcategory_code = code ORDER BY code)
          |)
          |SELECT (SELECT code FROM categories WHERE code={category_code}) AS category_code, code, level FROM t
          |UNION ALL
