...
import uk.ac.ncl.openlab.intake24.api.data.SurveyAliasCredentials
...
object Intake24CredentialsUtil {
  implicit def asSimpleCredentials(credentials: SurveyAliasCredentials) = Credentials(SurveyUserAlias(credentials.surveyId, credentials.userName).toString(), credentials.password)

  implicit def fromSimpleCredentials(credentials: Credentials) = {
    val key = SurveyAliasUtils.fromString(credentials.identifier)

    SurveyAliasCredentials(key.surveyId, key.userName, credentials.password)
  }

  implicit val jsonFormat = Json.format[SurveyAliasCredentials]
}